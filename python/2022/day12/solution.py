...
class Part2:
    ...
    def part2(self):
        ...
        for a in aNodes:
            ...
            self.visited.clear()
            self.start = a
            steps, self.start = self.solve("e", 0)

            if self.start == None:
                continue

            self.visited.remove(self.start)
            self.nodes_roamed = steps
            steps, _ = self.solve("E", steps)
            self.min_steps = min(steps, self.min_steps)
        print(f"Steps: ", self.min_steps)
    ...
    def solve(self, final_char: str, steps: int):
        self.visited.clear()
        ...
        while True:
            ...
            if node not in self.visited:
                ...
                if self.grid[x][y] == final_char:
                    ...
                    log(f"Steps '{final_char}' : ", min_steps)
                    log(f"Nodes Roamed: '{self.nodes_roamed}'")
