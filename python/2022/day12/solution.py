...
def findPath(grid: list[list[str]], visited: list[list[bool]], char: str, path: int, i: int, j: int, min_dist: int) -> int:
    ...
    if (i > 0 and grid[i-1][j]):
        min_dist = min(min_dist, findPath(grid, visited, char, path+1, i-1, j, min_dist));
    # down
    if (i < rows-1 and grid[i+1][j]):
        min_dist = min(min_dist, findPath(grid, visited, char, path+1, i+1, j, min_dist));
    # left
    if (j > 0 and grid[i][j-1] in [ chr(ord(grid[i][j])+1), grid[i][j] ]):
        min_dist = min(min_dist, findPath(grid, visited, char, path+1, i, j-1, min_dist));
    # right
