...
class DumboOctopus:
    ...
    def findAdjacentOctopuses(self):
        ...
        topRightOctopus = (
            DumboOctopus.octopuses[i - 1][j + 1] if (top and right) else None
        )
        bottomRightOctopus = (
            DumboOctopus.octopuses[i + 1][j + 1] if (bottom and right) else None
        )
        topLeftOctopus = (
            DumboOctopus.octopuses[i - 1][j - 1] if (top and left) else None
        )
        bottomLeftOctopus = (
            DumboOctopus.octopuses[i + 1][j - 1] if (bottom and left) else None
        )
        self.adjacentOctopuses = [
            leftOctopus,
            rightOctopus,
            topOctopus,
            bottomOctopus,
