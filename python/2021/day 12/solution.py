	...
	def findPaths(cls, curr_path, have_time=False):
		...
		for cave in c.connectedCaves:
			...
			if have_time:
				...
				cond = cave.name != 'start' and (not cave.small or cave.small and
					(not is_cave_visited_twice or not cave.name in curr_path))
			else:
				cond = (not cave.small or 
					(cave.small and not cave.name in curr_path))
